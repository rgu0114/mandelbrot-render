...
void update_window_title() {
    char title[1024];
    double centerX = ri.x0 + ri.step * IMG_SIZE / 2;
    double centerY = ri.y0 + ri.step * IMG_SIZE / 2;
    sprintf(title, "Mandelbrot Set (prec = %.5lf, iter = %d) at (%.6lf, %.6lf)",
            ri.prec, ri.max_iter, centerX, centerY);
...
void update_image(mouse_state_t ms) {
    ...
    // Update parameters
    double tmp = ri.step * x;
    ri.x0 += tmp;
    tmp = ri.step * y;
    ri.y0 += tmp;
    ri.step *= (1.0 * size / IMG_SIZE);

    // Render image
    auto calc_procedure = [](render_t ri, int id, int thread_num) {
        int cur_iter = render(
            ri,
            [=](int row) {
    ...
    for (int i = 0; i != THREAD_NUM; ++i)
        std::thread(calc_procedure, ri, i, THREAD_NUM).detach();
}

cairo_surface_t *cairo_get_surface() {
    int stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, IMG_SIZE);
...
void mouse_hit_event(GtkWidget *widget, GdkEventButton *event, gpointer data) {
...
void mouse_motion_event(GtkWidget *widget, GdkEventMotion *event, gpointer data) {
    ms.x = event->x;
    ms.y = event->y;
    gtk_widget_queue_draw(widget);
}
